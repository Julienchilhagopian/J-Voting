...
public class ColumnsDefaultGUI extends ProfileDefaultGUI {
    ...
    private static final Logger LOGGER = LoggerFactory
                    .getLogger(ColumnsDefaultGUI.class.getName());
    protected int sourceX = 0;
    protected int sourceY = 0;
    protected ViewerCell cellBeingDragged = tableViewer
                    .getCell(new Point(0, 0));
    ...
    /**
     * Implements dragging and dropping a cell at a time within the same
     * column.<br>
     * When moving it up, the cell it is dropped on and those below will go down
     * one cell. <br>
     * When moving it down, the cell it is dropped on and those above will go up
     * one cell.
     */
    public void handleDragAndDrop() {
        ...
        table.addListener(SWT.MouseDown, new Listener() {
            ...
            public void handleEvent(Event event) {
                ViewerCell viewerCell = tableViewer
        ...
        table.addListener(SWT.MouseUp, new Listener() {
            ...
            public void handleEvent(Event event) {
                if (cellBeingDragged == null)
                    return;
                ViewerCell destinationCell = tableViewer
                                .getCell(new Point(event.x, event.y));
                if (destinationCell != null
                                && destinationCell != cellBeingDragged) {
                    // if moving from one voter to another
                    if (cellBeingDragged.getColumnIndex() != destinationCell
                                    .getColumnIndex()) {
                        MessageBox messageBox = new MessageBox(mainShell,
                                        SWT.OK);
                        messageBox.setText("Warning");
                        messageBox.setMessage(
                                        "You can't move alternatives between voters !");
                        messageBox.open();
                    } else {// if moving cell within the same column
                        ViewerCell currentCell = tableViewer
                                        .getCell(new Point(sourceX, sourceY));
                        String cellBeingDraggedText = cellBeingDragged
                                        .getText();
                        // if source is over destination in the table
                        if (cellBeingDragged.getBounds().y < destinationCell
                                        .getBounds().y) {
                            while (currentCell.getBounds().y != destinationCell
                                            .getBounds().y) {
                                LOGGER.debug("Current cell : {} replaced by {}",
                                                currentCell.getText(),
                                                currentCell.getNeighbor(
                                                                ViewerCell.BELOW,
                                                                false)
                                                                .getText());
                                currentCell.setText(currentCell
                                                .getNeighbor(ViewerCell.BELOW,
                                                                false)
                                                .getText());
                                currentCell = currentCell.getNeighbor(
                                                ViewerCell.BELOW, false);
                            }
                            destinationCell.setText(cellBeingDraggedText);
                        } else {// if source is underneath destination in the
                                // table
                            while (currentCell.getBounds().y != destinationCell
                                            .getBounds().y) {
                                LOGGER.debug("Current cell : {} replaced by {}",
    ...
    public void save(String outputFile) throws DuplicateValueException, EmptySetException{
        ...
        try (OutputStream outputStream = new FileOutputStream(file)) {
            ...
            MessageBox dialog = new MessageBox(mainShell,
