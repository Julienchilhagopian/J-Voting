...
public class SOCColumnsGUI extends ColumnsDefaultGUI {
    ...
    private static final Logger LOGGER = LoggerFactory
                    .getLogger(SOCColumnsGUI.class.getName());
    private String alternativeToAdd;
    private final Label label = new Label(mainShell, SWT.NULL);
    private final Text newAlternativeText = new Text(mainShell,
    ...
    public void addAlternative(String alternative) {
        ...
        if (strictProfile.getAlternatives().contains(
            ...
            List<String> altForEveryone = Collections
                            .nCopies(strictProfile.getNbVoters(), alternative);
            item.setText(altForEveryone
                            .toArray(new String[strictProfile.getNbVoters()]));
    ...
    public void populateRows() throws DuplicateValueException, EmptySetException {
        ...
        for (int i = 0; i < strictProfile.getNbAlternatives(); i++) {
            ...
            List<Alternative> ithAlternatives = strictProfile
                            .getIthAlternatives(i);
            for (Alternative alt : ithAlternatives) {
                alternatives.add(alt.toString()); // convert alternatives in the
                                                  // list to strings
            }
            TableItem item = new TableItem(table, SWT.NONE);
            item.setText(alternatives.toArray(
                            new String[strictProfile.getNbAlternatives()])); // create
                                                                             // a
                                                                             // row
                                                                             // with
                                                                             // ith
